...
class PPOGuided:
	...
	def __init__(self, env, **hyperparameters):
		...
		self.cov_var = torch.full(size=(self.act_dim,), fill_value=0.001)
		self.cov_mat = torch.diag(self.cov_var)
		self.cov_var_or = torch.full(size=(self.act_dim,), fill_value=1.0)
		self.cov_mat_or = torch.diag(self.cov_var_or)
		self.obs_count = 0
		self.index_count = 0
		...
		self.logger = {
			...
			'batch_rews_net': [],   # Episodic returns in a neural network
	...
	def learn(self, env_name,failure_observations,subpolicy):
		...
		while i_so_far < self.training_step:                                                                       # ALG STEP 2
			...
			if i_so_far % self.save_freq == 0:
				print(surr1)
				self.test_intermediate_policy(True)
	...
	def rollout(self,subpolicy,failure_observations):
		...
		batch_rtgs = []
		batch_lens = []
		# Two new metrices for guided learning
		batch_rews_net = []
		batch_infractions = []
		...
		while t < self.timesteps_per_batch:
			...
			ep_infractions = []	# infractions collected per episode
			...
			for ep_t in range(self.max_timesteps_per_episode):
				...
				#Get the actions predicted by the mathametical model
				a_predicted, delta = self.predicted_control()

				t += 1 # Increment timesteps ran this batch so far

				# Track observations in this batch
				batch_obs.append(obs)
				#print(self.actor(obs))
				...
				if self.discrete:
					...
					#action_net, _ = self.get_action(obs)
					action_net = self.actor(obs).detach().numpy()
					action, log_prob = self.get_action_guided([a_predicted,delta])

				# Execute the lookahead state to check the stability and safety criteria
				stability_criteria, safety_criteria, rew, done, info = self.env.step_lookahead(action_net)
				# Consider the action from the network
				if(stability_criteria and safety_criteria):
					action, log_prob = self.get_action(obs)


				if(abs(round(float(action_net[0]),1))<abs(round(float(action[0]),1))):
					count_infractions_acc = count_infractions_acc+1
				if(abs(round(float(action_net[1]),1)) < abs(round(float(action[1]),1))-0.2):
				...
				#This is the reward obtined for executing the lookahead step
				#_, rew_net, _, _ = self.env.step_lookahead(action_net)
			...
			#print(f'count_infractions =============== {count_infractions}======count_infractions_acc===={count_infractions_acc}=======count_infractions_steer===={count_infractions_steer}')
		...
		#print(f'batch_infractions =============== {batch_infractions}')
		...
		batch_rews_net.append(self.test_intermediate_policy(False))                                                              # ALG STEP 4

		# Log the episodic returns and episodic lengths in this batch.
		self.logger['batch_rews'] = batch_rews
		self.logger['batch_rews_net'] = batch_rews_net
		self.logger['batch_infractions'] = batch_infractions
		self.logger['batch_lens'] = batch_lens

		return batch_obs, batch_acts, batch_log_probs, batch_rtgs, batch_lens

	def predicted_control(self):
	...
	#Test the original policy after every iteration of training
	def test_intermediate_policy(self,render):
		ep_rews_net = []
		obs = self.env.reset()
		done = False
		act_list = []
		while not done:
			...
			action_net = self.actor(state).detach().numpy()
			obs, rew_net, done, info = self.env.step(action_net)
			ep_rews_net.append(rew_net)
			act_list.append(action_net)
		if render:
			self.env.render(act_list)
		return ep_rews_net	
	...
	def get_action_guided(self, action_pred):
		"""
			Queries an action from the actor network, should be called from rollout.
		...
		"""
		# Query the actor network for a mean action
		mean = torch.tensor(np.array(action_pred,dtype=np.float32))
		#print(self.actor(obs))
	...
	def evaluate(self, batch_obs, batch_acts):
		...
		if self.discrete:
			...
			dist = MultivariateNormal(mean, self.cov_mat_or)
	...
	def _log_summary(self):
		...
		avg_ep_rews_net = np.mean([np.sum(ep_rews_net) for ep_rews_net in self.logger['batch_rews_net']])
		...
		if(i_so_far > 300):
			avg_ep_rews_net = np.clip(avg_ep_rews_net,-50,700)

		# Round decimal places for more aesthetic logging messages
		avg_ep_lens = str(round(avg_ep_lens, 2))
		avg_ep_rews = str(round(avg_ep_rews, 2))
		#avg_ep_rews_net = str(np.clip(round(avg_ep_rews_net, 2),-400,200))
		avg_ep_rews_net = str(round(avg_ep_rews_net, 2))
		avg_ep_infractions = str(round(avg_ep_infractions, 2))
		avg_actor_loss = str(round(avg_actor_loss, 5))

		writer.add_scalar("Average Episodic Return", int(float(avg_ep_rews_net)), t_so_far)
		...
		print(f"Average Episodic Return Network: {avg_ep_rews_net}", flush=True)
