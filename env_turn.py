...
from cbf_clf_helper import clf_control
...
class KinematicBicycleGymTurn(gym.Env):

	def __init__(self):
		super(KinematicBicycleGymTurn, self).__init__()
		...
		self.action_space = spaces.Box(-0.6, 1.0, (2,), dtype=np.float32)
	...
	def reset(self):
		...
		self.x_ego = 30.0 #Ego vehicle's position
		self.y_ego = 20.0#+rand.randint(-5,5) #Ego vehicle's position
		...
		self.v_ego = 30 #Ego vehicle's velocity
		self.delta_ego = 0.0
	...
	def render(self,actionlist):
		...
		self.road = plt.Rectangle((30, 0), 50,50, color='gray', fill=False, linewidth=40)

		#road = plt.Circle((0, 0), 50, color='gray', fill=False, linewidth=30)
		self.ax.add_patch(self.road)
		self.ax.plot(self.path.px, self.path.py, '--', color='gold')
