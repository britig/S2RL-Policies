...
class Path:
	...
	def __init__(self):
		...
		dir_path = 'data/waypointscutindata.csv'
...
class KinematicBicycleGymCutIn(gym.Env):

	def __init__(self):
		super(KinematicBicycleGymCutIn, self).__init__()
		...
		self.action_space = spaces.Box(-1.0, 1.4, (2,), dtype=np.float32)
	...
	def reset(self):
		...
		self.delta_ego = 0.0
		self.v_desired = 10 #Desired Velocity
		self.v_lead = 0.0
	...
	def step_lookahead(self,action):
		...
		distance_sc = math.sqrt((x_ego - self.car_sc.x)**2 + (y_ego - self.car_sc.y)**2)
		...
		if distance_fc > sigma_1 and distance_sc > sigma_2:
...
if __name__=='__main__':
	...
	while not done:
		...
		elif(round(env.x_ego) == round(env.car_sc.x,1)):
			distance = env.car_sc.y - env.y_ego
			vel_lead = env.car_sc.v
			a_predicted_cbf = cbf_control(env.v_ego,vel_lead,distance)
			print(f'distance after lc ======= {env.distance_sc}==speed===== {env.v_ego}')
			if(distance < 8):
				throttle = a_predicted_cbf
		# Car doing a lane change
		else:
			distance = math.sqrt((env.x_ego - env.car_sc.x)**2 + (env.y_ego - env.car_sc.y)**2)-1.5
			vel_lead = env.car_sc.v
			a_predicted_cbf = cbf_control(env.v_ego,vel_lead,distance)
